...
namespace OpenRA.Mods.SP.Traits.BotModules
{
	...
	public class PowerDownBotModuleSPInfo : ConditionalTraitInfo
	{
		[Desc("Delay (in ticks) between toggling powerdown")]
		public readonly int Interval = 150;

		public override object Create(ActorInitializer init) { return new PowerDownBotModuleSP(init.Self, this); }
	}

	public class PowerDownBotModuleSP : ConditionalTrait<PowerDownBotModuleSPInfo>, IBotTick
	{
		...
		public PowerDownBotModuleSP(Actor self, PowerDownBotModuleSPInfo info)
			: base(info)
		{
			world = self.World;
			player = self.Owner;
