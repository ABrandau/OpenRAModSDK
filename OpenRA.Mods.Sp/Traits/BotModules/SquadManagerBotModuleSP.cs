...
using OpenRA.Mods.SP.Traits.BotModules.Squads;
using OpenRA.Support;
using OpenRA.Traits;

namespace OpenRA.Mods.SP.Traits.BotModules
{
	[Desc("Manages AI squads.")]
	public class SquadManagerBotModuleSPInfo : ConditionalTraitInfo
	{
		...
		public override object Create(ActorInitializer init) { return new SquadManagerBotModuleSP(init.Self, this); }
	...
	public class SquadManagerBotModuleSP : ConditionalTrait<SquadManagerBotModuleSPInfo>, IBotEnabled, IBotTick, IBotRespondToAttack, IBotPositionsUpdated, IGameSaveTraitData
	{
		...
		public List<SquadSP> Squads = new List<SquadSP>();
		...
		public SquadManagerBotModuleSP(Actor self, SquadManagerBotModuleSPInfo info)
			: base(info)
		{
			World = self.World;
			Player = self.Owner;
		...
		SquadSP GetSquadOfType(SquadTypeSP type)
		{
			return Squads.FirstOrDefault(s => s.Type == type);
		}

		SquadSP RegisterNewSquad(IBot bot, SquadTypeSP type, Actor target = null)
		{
			var ret = new SquadSP(bot, this, type, target);
			Squads.Add(ret);
			return ret;
		}
		...
		void FindNewUnits(IBot bot)
		{
			...
			foreach (var a in newUnits)
			{
				...
				{
					var air = GetSquadOfType(SquadTypeSP.Air);
					if (air == null)
						air = RegisterNewSquad(bot, SquadTypeSP.Air);

					air.Units.Add(a);
				}
				else if (Info.NavalUnitsTypes.Contains(a.Info.Name))
				{
					var ships = GetSquadOfType(SquadTypeSP.Naval);
					if (ships == null)
						ships = RegisterNewSquad(bot, SquadTypeSP.Naval);
		...
		void CreateAttackForce(IBot bot)
		{
			...
			{
				var attackForce = RegisterNewSquad(bot, SquadTypeSP.Assault);
		...
		void TryToRushAttack(IBot bot)
		{
			...
			foreach (var b in allEnemyBaseBuilder)
			{
				...
				if (AttackOrFleeFuzzySP.Rush.CanAttack(ownUnits, enemies))
				{
					var target = enemies.Any() ? enemies.Random(World.LocalRandom) : b;
					var rush = GetSquadOfType(SquadTypeSP.Rush);
					if (rush == null)
						rush = RegisterNewSquad(bot, SquadTypeSP.Rush, target);
		...
		{
			var protectSq = GetSquadOfType(SquadTypeSP.Protection);
			if (protectSq == null)
				protectSq = RegisterNewSquad(bot, SquadTypeSP.Protection, attacker);
		...
		void IGameSaveTraitData.ResolveTraitData(Actor self, List<MiniYamlNode> data)
		{
			...
			if (squadsNode != null)
			{
				...
				foreach (var n in squadsNode.Value.Nodes)
					Squads.Add(SquadSP.Deserialize(bot, this, n.Value));
			}
		}
	}
}
